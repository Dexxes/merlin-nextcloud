<template>
	<section class="test-panel">
		...
		<p class="test-panel__hint">
			{{ t('merlin', 'Fetches an article of this website with the rules currently shown — including unsaved changes — and reports how many elements each rule matched. Nothing is saved.') }}
		</p>
			...
			<table v-if="report.trace.length" class="test-panel__trace">
				...
				<tbody>
					<tr
						...
						<td>{{ entry.origin === 'admin' ? t('merlin', 'own') : t('merlin', 'built-in') }}</td>
...
<script>
import { testFilter } from '../../api/contentFilters.js'

export default {
	name: 'FilterTestPanel',
	...
	methods: {
		...
		async run() {
			...
			this.report = null
			try {
				// Entwurf mitschicken: der Admin soll einen XPath prüfen können,
				// ohne ihn vorher instanzweit für alle Nutzer zu aktivieren.
				...
				// Auch bei einem gescheiterten Abruf kann der Trace schon Regeln
				// enthalten – der Fehler kam dann erst nach dem Filtern.
...
/* Der Inhalt hat sanitizeHtml() im Extractor durchlaufen (Allowlist für Tags
   und Attribute, javascript:-Schemata gefiltert) – es ist derselbe HTML-Stand,
   den der Reader anzeigt. */
.test-panel__article {
	max-height: 420px;
	overflow-y: auto;
	border: 1px solid var(--color-border);
